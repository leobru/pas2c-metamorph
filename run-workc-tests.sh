#!/bin/bash
# Test runner for the tests-c/ C-declarator corpus through the SELF-HOSTED
# work-c.p2c compiler (workc.bin, built by work-c.sh), via runworktest.sh
# with WORK_MODULE=workc. Mirrors runtests.sh -work, but against tests-c/
# and the workc compiler module instead of tests/ and work.bin.
...
TESTS_DIR="tests-c"
RESULTS_DIR="test_results_workc"
RUNNER="runworktest.sh"
export WORK_MODULE=workc
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}work-c.p2c (self-hosted) / tests-c Suite${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
